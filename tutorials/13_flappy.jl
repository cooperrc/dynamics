...
# ╔═╡ be6b3890-2ddd-11f1-8686-7f2956aac94a
using Symbolics, DifferentialEquations, ModelingToolkit, Plots

# ╔═╡ d6873084-e93e-426f-a4e6-5cc723eec6fe
using ModelingToolkit: t_nounits as t, D_nounits as D

# ╔═╡ b137137a-f438-4817-af62-0a0faf294a67
md"""
...
# ╔═╡ c3d5ec8f-23bc-436a-8c1d-61caa7ae3e12
md"""## Setting up functions and constants
...
> Note: `x, y, vx, vy` are all functions of time, but because we are not solving a set of differential equations we want to keep them as indepenent variables e.g. `@variables x` **not** `@variables x(t)`. 
"""

# ╔═╡ 14bbbf73-5671-4117-a9c9-040277702dfe
begin
	@variables x, y, vx, vy
	q = [x, y]
	dq = [vx, vy]
...
# ╔═╡ e8334c05-4786-4a80-9da1-c12423a4492a
md""" Here, we can see the two constraints on motion written as the constraint equation, $C(q,~t)$ and its Jacobian, $\frac{\partial C}{\partial q}$. Each of these printed equation arrays are formed with the Julia [Computer Algebra System](https://en.wikipedia.org/wiki/Computer_algebra_system) so they can be rearranged and used in algebraic+calculus expressions."""

# ╔═╡ 7e0263d8-36b8-4353-8649-864cad83dc13
C(q, 0.1)

# ╔═╡ 7a56f213-5e8b-4a7a-98d6-0ad968f8c4d4
dCdq(q, t) = Symbolics.jacobian(C(q, t), q)

# ╔═╡ 59605d05-34fd-4247-8ab9-c10fcafe8bd8
dCdq(q, t)*D.(q) .~ -Symbolics.derivative(C(q,t), t)


# ╔═╡ ade256b5-628d-4edf-8d4c-11ced9195a4e
md"""## Solving for _actual_ positions and velocities
...
# ╔═╡ 80af86ea-d1c8-48b6-8bb7-6791e7146dfb
begin
	N = 100
	tN = LinRange(0, 4, N)
...
# ╔═╡ 2d2fc70b-7ade-48df-94ad-cac05c4e961b
md""" ## Wrapping up

The process here depends upon identifying constraints and degrees of freedom carefully, but it _can_ scale to as many constraints and degrees of freedom as our system might need. The code will change a little, but if we can define a system where #DOFs == # constraints, there are no differential equations and the problem is a set of nonlinear + linear algebra equations at each step in time. 

Plus, we now have a Flappybird velocity profiler ;)

"""

# ╔═╡ d6dfd3d5-23aa-4bd1-95cb-0111b5248766
function plot_path_with_velocity!(
    position::AbstractMatrix,
    velocity::AbstractMatrix;
    index::Integer = size(position, 1),
    half_width::Real = 10.0,
    half_height::Real = half_width,
    arrow_scale::Real = 1.0,
    plt = nothing
)
    @assert size(position, 2) == 2 "position must be N×2 (x,y)"
    @assert size(velocity, 2) == 2 "velocity must be N×2 (vx,vy)"
    @assert size(position, 1) == size(velocity, 1) "position/velocity must share N"
    @assert 1 ≤ index ≤ size(position, 1) "index out of bounds"

    current_x, current_y = position[index, 1], position[index, 2]
    current_vx, current_vy = velocity[index, 1], velocity[index, 2]

    plt === nothing && (plt = plot(; legend=false, aspect_ratio=:equal))

    plot!(plt, position[:, 1], position[:, 2]; lw=2, c=:steelblue)              # path
    scatter!(plt, [current_x], [current_y]; ms=6, c=:crimson)                  # current point
    quiver!(plt, [current_x], [current_y];
            quiver=([arrow_scale*current_vx], [arrow_scale*current_vy]),
            c=:crimson, lw=2)                                                  # velocity arrow

    xlims!(plt, current_x - half_width,  current_x + half_width)               # center view
    ylims!(plt, minimum(position[:,2]) - half_height, 
		   maximum(position[:,2]) + half_height)
...
manifest_format = "2.0"
project_hash = "fc04801e3551f3630c2d583797afeac4859136aa"

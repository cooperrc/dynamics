...
# ╔═╡ b92806a8-4fa2-424e-884f-29fd41bdeac7
using NonlinearSolve,Plots, Latexify

# ╔═╡ e11f3d80-399c-494e-9a4d-d4ee43e8aa15
md"""# Solving Kinematic Constraints

![Reference frames for door opening](https://raw.githubusercontent.com/cooperrc/dynamics/refs/heads/main/images/door-frames.png)

Moving door and the reference frames to define its motion in body-fixed and global coordinates. 

[![Solving for door opening kinematics](https://img.youtube.com/vi/u27rJ7I0d3g/0.jpg)](https://www.youtube.com/watch?v=u27rJ7I0d3g "Solving for door opening kinematics - click to watch")
...
We use these hinge constraints to build $\mathbf{C}(\mathbf{q},~t)$

"""

# ╔═╡ 0c6812a2-69bd-4e71-9b48-9fa53a4d1a2f
md"""
...
# ╔═╡ 74b0485f-b6da-48ba-93eb-72f547b5aa7c
begin
	#q = [x_1, y_1, θ_1]
	W = 1
	T = 3e-2
...
# ╔═╡ f467b4b6-ffad-486e-9688-1d1a6dd73e1d
begin
...
# ╔═╡ 5061cf25-995e-40b9-a09c-b550f2cfa4fb
plot(time, q_all)

# ╔═╡ 5a02feee-5809-4e2b-a757-b21d03d80f82
C(q_all[51, :], time[51])

# ╔═╡ 376d75c0-f335-44b3-8572-f9af5dd70308
md"""

## Solving the System speeds
At each timestep:
1. **Position solve:** find $q$ such that $C(q,t)=0$
2. **Velocity solve:** solve linear system $C_q \dot{q} = -C_t$
"""

# ╔═╡ 324baace-0e15-4183-a485-ac76a04c7266
begin
...
# ╔═╡ 63fbdafa-ce27-44bd-b7f3-d9768c2c3763
plot(time, dq_all)

# ╔═╡ d236fd32-4fde-477a-a900-efcc49e7d60b
function plot_oriented_rectangle!(
    pose_q::AbstractVector,       # [position_x, position_y, orientation_theta]
    rectangle_width::Real,        # total width of the rectangle
    rectangle_thickness::Real;    # total height/thickness of the rectangle
    face_color = :lightblue,
    edge_color = :black,
    face_alpha = 0.5
)
    position_x, position_y, orientation_theta = pose_q
    half_width = rectangle_width / 2
    half_thickness = rectangle_thickness / 2

    local_x = [-half_width,  half_width,  half_width, -half_width]
    local_y = [-half_thickness, -half_thickness, half_thickness,  half_thickness]

    world_x = position_x .+ cos(orientation_theta) .* local_x .- sin(orientation_theta) .* local_y
    world_y = position_y .+ sin(orientation_theta) .* local_x .+ cos(orientation_theta) .* local_y

    shape = Shape(world_x, world_y)
    return plot!(shape, c = face_color, linecolor = edge_color, alpha = face_alpha, legend = false)
end

# ╔═╡ 20b642f2-9fae-498a-8174-0d0529099ca7
md""" ## Animate the door motion

In this next step, we use `plot_oriented_rectangle` to show the door in a given position and orientation. The input to the Julia Assistant in copilot was, 

> "I need you to give me a julia function that plots a rectangle given its x,y position and orientation, theta in a vector q=[x, y, theta] and let me define its width and thickness for the plot"

Then, we use the `@gif` loop to watch the door open over time. 

"""


# ╔═╡ d56a4004-38dd-479f-a275-e02e25773324
...
# ╔═╡ 24bdda52-dd39-4be7-bafc-0dd5b501868d
...
manifest_format = "2.0"
project_hash = "d9dcbf03f723cbee3c8032f61b7f4c80fab2ab5e"
